...
public class RaindropsScene extends SimpleMotionScene {
	...
	public RaindropsScene(PApplet p, int code, int fadeSpeed) {
		super(p, code, fadeSpeed);
		
		fieldList.add(new Field(p,  20, 200, 400, 400, -.1f, .6f,
				"rain1.mp3", 20, true));
		fieldList.add(new Field(p,  600, 400, 100, 100, .6f, .5f,
				"rain4.aiff", 2, true));
		fieldList.add(new Field(p,  300, 300, 100, -210, -.6f, .7f,
				"rain4.aiff", 1,true));
		fieldList.add(new Field(p,  70, 500, 200, -150, .2f, .9f,
				"rain3.mp3", 10,true));
		fieldList.add(new Field(p,  30, 300, 20, -150, .3f, .4f,
				"rain3.mp3", 20,true));
		fieldList.add(new Field(p, 10, 200, 0, 100, .1f, .6f,
				"rain2.wav", 30,true));
		fieldList.add(new Field(p, 150, 200, 200, -200, .01f, .3f,
				"rain2.wav", 20,true));
		
		
		fieldList.add(new Field(p, 2, 500, p.width/2, p.height/2, 0, 0,
...
//			g.fill(200, 0, 0);
//			g.ellipse(data.LEFT_FOOT.x, PApplet.map(data.LEFT_FOOT.z, 1600, 4000, 1200, 0), 50, 50);
		}
	}
	...
	private void checkForPlatsch() {
		...
		if (data.LEFT_FOOT.y > 720 || data.RIGHT_FOOT.y > 720 ) {
			if(footUp) {
			 platschList.add(new Platsch(p, data.LEFT_FOOT.x, PApplet.map(data.LEFT_FOOT.z, 1600, 4000, 1200, 0)));
			 footUp = false;
			}
		}
...
*/
	...
	private void prooveForSchirm() {
		float frameDifLH = Math.abs(oldLeftHandY - data.LEFT_HAND.y);
		float frameDifRH = Math.abs(oldRightHandY - data.RIGHT_HAND.y);

		float difRH = data.NECK.y + data.TORSO.y - data.NECK.y
				- data.RIGHT_HAND.y;
		float difLH = data.NECK.y + data.TORSO.y - data.NECK.y
				- data.LEFT_HAND.y;
		float difTH = Math.abs(data.TORSO.y - data.HEAD.y);
...
//		System.out.println(schirmHeight);
