...
public class OffCenter2 extends PApplet {
	...
	private PVector[] objects = new PVector[2000];
	...
	private PVector objectPosition = new PVector(-40, 220, 165);
	private PVector objectSize = new PVector(80, 95, 50);
	...
	public void setup() {
	  ...
	  for(int i = 0; i < objects.length; i++) {
		  float xx = random(-10000, 10000), yy = random(-10000, 10000), zz = random(-10000, 10000);
			objects[i] = new PVector(xx, yy, zz);
		}
	}
...
//		g.applyMatrix(M);
		...
		g.beginCamera();
//		g.camera(pe.x*pe.z, pe.y*pe.z, 0, pe.x*pe.z, pe.y*pe.z, -1, 0, 1, 0);
//		g.translate(-pe.x, -pe.y, -pe.z);
		
		gl.glMatrixMode(GL2.GL_MODELVIEW);
		gl.glLoadIdentity();
		GLU glu = GLU.createGLU(gl);
//		glu.gluLookAt(pe.x, pe.y, pe.z, pe.x, pe.y, -1, 0, 1, 0);
	}
...
//	  drawViewAlphaAndLine();
	...
	public void drawHoloObject() {
		pushStyle();
			...
			pushMatrix();
				noStroke();
				for(int i = 0; i < objects.length; i++) {
					fill(map(objects[i].z, -10000, 10000, 100, 255), 0, 100);
					pushMatrix();
			...
			pushMatrix();
				translate(objectPosition.x+(objectSize.x/2f), objectPosition.y, objectPosition.z+(objectSize.z/2f));
	...
	//			rotateX(verticalViewAlpha);
				...
				box(objectSize.x, objectSize.y, objectSize.z);
			...
			pushMatrix();
				noFill();
				stroke(0, 200, 200);
				translate(objectPosition.x+200, objectPosition.y+200, objectPosition.z+200);
				sphere(20);
			popMatrix();
	...
	public static void main(String args[]) {
		...
		"--display=0",
		"offcenterprojection2.OffCenter2"
	    });
	}
