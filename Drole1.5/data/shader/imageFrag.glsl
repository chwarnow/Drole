...
vec3 n = normalize(normal);
	...
	// ambient term
	vec4 ambient = mat_ambient * (gl_Color*0.1);//gl_Color;// gl_LightSource[0].ambient;
	
	// diffuse color
	vec4 kd = mat_diffuse * gl_Color*1.0;// gl_LightSource[0].diffuse;
	
	// specular color
	vec4 ks = mat_specular * (gl_Color*1.5);// gl_Color;// gl_LightSource[0].specular;
