...
public class LPhysics02a extends PApplet {
	...
	public void initGUI() {
	  ...
	  ui.addSlider("isoThreshold",1,30,isoThreshold,20,20,100,14).setLabel("iso threshold");
...
//					  VerletParticle s = new VerletParticle(center.x(), center.y(), center.z());
					  ...
					  physics.addSpring(new VerletSpring(s, ss, REST_LENGTH/5f, 0.001f));
				  }
				  */
			  }
			  for(int i = 0; i < numFirstBranch; i++) {
				  if(i == numFirstBranch-1) {
					  ...
					  physics.addSpring(new VerletSpring(physics.particles.get(i), physics.particles.get(i+1), REST_LENGTH, 0.001f));
				  }
		  ...
		  if (random(1)<0.8 && numP<NUM_PARTICLES) {
		    ...
		    if(physics.particles.size() > 1) physics.addSpring(new VerletSpring(physics.particles.get(0),p,REST_LENGTH, 0.0002f));
		  }
		  */
		  /*
